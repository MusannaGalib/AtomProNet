    ...
    while True:
        ...
        if option == '1':
            ...
            if post_processing_option == '1':
                ...
                # Construct the full path for MD_benchmarking.py (adjust if necessary)
                md_file = os.path.join(script_dir, "MD_benchmarking.py")
                ...
                if not os.path.isfile(md_file):
                    ...
                    try:
                        # Run MD_benchmarking.py interactively (using subprocess.call so input() works)
                        subprocess.call(['python', md_file])
                    except Exception as e:
                        print(f"An error occurred while running MD_benchmarking.py: {e}")
